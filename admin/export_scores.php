...
// 2. 获取课程基本信息用于文件名（修复关联 courses.id）
$info_sql = "SELECT c.class_name, co.course_name 
             FROM schedule s 
             JOIN classes c ON s.class_id = c.class_id 
             JOIN courses co ON s.course_id = co.id  -- 关联 courses 自增 int 主键
             WHERE s.id = ?";
$info_stmt = $conn->prepare($info_sql);
$info_stmt->bind_param("i", $sch_id); // 绑定 int 类型
$info_stmt->execute();
$info = $info_stmt->get_result()->fetch_assoc();
// 处理无课程信息的场景
$fileNamePrefix = ($info['class_name'] ?? '未知班级') . "_" . ($info['course_name'] ?? '未知课程');
...
$stmt->bind_param("i", $sch_id); // 绑定 int 类型
$stmt->execute();
...
if ($result && $result->num_rows > 0) {
    ...
    $sheet->setCellValue('A2', '暂无学生成绩数据');
    $sheet->mergeCells('A2:G2'); // 合并单元格
}
